...
def read_data(filename):
    ...
    return id_, gender, text

_, trn_gender, trn_text = read_data("dll-a2.train")
tst_id, _, tst_text = read_data("dll-a2.test")


#tfidf = TfidfVectorizer(max_features=500, lowercase=True, analyzer='word', stop_words= 'english',ngram_range=(1,1))
#dat_tfIdf = tfidf.fit_transform(trn_text)



# Convert csr matrices to sparse tensors
vec = CountVectorizer(ngram_range=(1,1), lowercase=True)
trn_x = vec.fit_transform(trn_text)
tst_x = vec.transform(tst_text)
...
# create y tensor with 
gender = []
for row in trn_gender:
    if(row == "male"):
        gender.append(0)
    else:
        gender.append(1)
y = torch.zeros(len(trn_gender), dtype=int)
for i in range(len(trn_gender)):        
    y[i] = gender[i]
